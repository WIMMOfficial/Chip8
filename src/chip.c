...
uint32_t timer_interrupt(Uint32 interval, void* params)
{
...
/*Set ir to the address NNN */ 
    case 0xA0:
      cpu.ir.WORD = (opcode.WORD & 0x0FFF);
	  sprintf(cpu.op_string,"LOAD IR, %02X", (opcode.WORD & 0x0FFF));
      break;
	  
	  
/*Jump to the address NNN plus V0*/
    case 0xB0:
      cpu.pc.WORD = cpu.ir.WORD + (opcode.WORD & 0x0FFF);
	  sprintf(cpu.op_string,"JMP IR + %02X", (opcode.WORD & 0x0FFF));
      break;
...
/*Draw a sprite at coordinate (VX, VY) that has a width of 8 pixels and a height of h pixels*/
    case 0xD0:
	  ...
	  for (int i = 0; i < h; ++i)
	  {
		tmp_byte = mem_read_byte(cpu.ir.WORD + i);
		coord_y = cpu.V[Y] + i;
	...
	case 0x1E:
	    cpu.ir.WORD += cpu.V[X];
		sprintf(cpu.op_string,"ADD IR %X", cpu.V[X]);
	  break;

		/*Set ip to the location of the sprite for the character in VX */
	case 0x29:
	  cpu.ir.WORD = cpu.V[X] * 5;
	  sprintf(cpu.op_string,"LOAD IR %X", cpu.V[X]);
	  break;
	...
	tmp_ir.WORD = cpu.ir.WORD;
	mem_write_word(tmp_ir.WORD, tmp_word.WORD);
	...
	break;
		...
		/*Store V0 to VX (including VX) in memory starting at address ir*/
	case 0x55:
	  reg_dump(X);
